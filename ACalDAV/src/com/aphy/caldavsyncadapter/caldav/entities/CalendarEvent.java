...
package com.aphy.caldavsyncadapter.caldav.entities;
...
import com.aphy.caldavsyncadapter.android.entities.AndroidEvent;
...
import com.aphy.caldavsyncadapter.Event;
...
import com.aphy.caldavsyncadapter.caldav.xml.MultiStatusHandler;
import com.aphy.caldavsyncadapter.caldav.xml.sax.MultiStatus;
import com.aphy.caldavsyncadapter.caldav.xml.sax.Prop;
import com.aphy.caldavsyncadapter.caldav.xml.sax.PropStat;
import com.aphy.caldavsyncadapter.caldav.xml.sax.Response;
...
import ch.punkt.mp02.dailyview.BuildConfig;
...
public class CalendarEvent extends Event {
    ...
    public CalendarEventData readContentValues() {
        CalendarEventData calendarEventData = new CalendarEventData();
        calendarEventData.setDtStart(this.getStartTime());
        calendarEventData.setTimeZone(this.getTimeZoneStart());

        if (this.getRRule() == null && this.getRDate() == null) {
            calendarEventData.setDtEnd(this.getEndTime());
            calendarEventData.setEndTimeZone(this.getTimeZoneEnd());
        } else {
            calendarEventData.setDuration(this.getDuration());
        }
        int AllDay = this.getAllDay();
        calendarEventData.setAllDay(AllDay);
        calendarEventData.setTitle(this.getTitle());
        calendarEventData.setSyncID(this.getUri().toString());
        calendarEventData.setETag(this.getETag());
        calendarEventData.setDescription(this.getDescription());
        calendarEventData.setLocation(this.getLocation());
        calendarEventData.setAccessLevel(this.getAccessLevel());
        calendarEventData.setStatus(this.getStatus());
        calendarEventData.setRdate(this.getRDate());
        calendarEventData.setRrule(this.getRRule());
        calendarEventData.setExRule(this.getExRule());
        calendarEventData.setExDate(this.getExDate());
        calendarEventData.setUID(this.getUid());
//        calendarEventData.setRawData(this.stringIcs);

        try {
            String dtstart = TimeUtil.getDate(calendarEventData.getDtStart());
            String exdate = calendarEventData.getExDate();
            if ("FREQ=WEEKLY".equals(calendarEventData.getRrule()) || "FREQ=DAILY".equals(calendarEventData.getRrule())) {
                if (calendarEventData.getExDate() != null) {
                    String[] strings = exdate.split(",");
                    for (int i = 0; i < strings.length; i++) {
                        if (strings[i].contains(dtstart)) {
                            return null;
                        }
                    }
                }
    ...
    private boolean parseIcs() throws CaldavProtocolException, IOException, ParserException {
        ...
        Log.e(TAG, "info lcError = " + lcError);
        if (!lcError) {
            ...
            if (!lcError) {
                ...
                if (BuildConfig.DEBUG)
                    Log.e(TAG, "info calendarComponent = " + calendarComponent);
            }
        }
    ...
    public CalendarEventData createmp02Event() {
        try {
            boolean lcBodyFetched = this.fetchBody();
            Log.i(TAG, "sync lcBodyFetched = " + lcBodyFetched);
            if (lcBodyFetched) {
                return readContentValues();
            } else {
                return null;
            }
        } catch (IOException e) {
